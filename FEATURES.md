...
**功能描述**:
- 支持图片(Sprite/Image)替代 Emoji 渲染
- 6 个独立可交互部位 (头、身、四肢)
- 独立的部位动画与触碰反馈
- 支持分层皮肤配置 (skin.json)

**技术实现**:
- 渲染: WebView CSS3 (Background Image / Sprite)
- 通信: `handlePartedModeSwitch` 协议扩展
- 配置: `skin.json` (type: "parted")
...
### 16. 热区 AI 混合响应 (Hybrid AI Interaction) ⭐⭐⭐⭐⭐

**状态**: ✅ 已完成 (2025-12-08)

**功能描述**:
- **精准热区**: 支持头部、身体、四肢独立热区检测 (基于 SVG/CSS)
- **混合响应策略**:
    - **本地快速响应 (0-7点击)**: 立即反馈，零延迟，包含心情分级文案
    - **随机 AI 介入 (30%概率)**: 增加不确定性和惊喜感，由宠物当前心情加成
    - **强制 AI 互动 (>8点击)**: 连续点击触发深度对话
- **心情感知**: 本地回复与 AI Prompt 均受宠物心情 (Happy/Normal/Sad) 影响

**技术实现**:
- 策略引擎: `usePetInteraction.js` (`shouldTriggerAI`)
- 本地词库: `usePetParts.js` (`LOCAL_PARTS_CONFIG`)
- 通信: `uniapp.sendDataToUni` (H5 -> App)
