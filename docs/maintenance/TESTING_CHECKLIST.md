...
### 🆕 [近期更新] 悬浮窗实体宠物 (Parted Pet Mode)
**状态**: 待验证 ✅
**触发条件**: 切换皮肤到 "经典幽灵 (分层版)" 或开启 "分层宠物模式" 开关
**测试要点**:
- 验证宠物无需重启即可切换为图片实体而非 Emoji
- 验证头、身体、四肢 (6个部位) 图片渲染正确 (Sprite模式)
- 点击各部位是否触发相应的独立动画和日志
- 测试长按部位的反馈

**相关文件**: 
- `static/pet.html`
- `static/skins/default-ghost/skin.json`
- `static/skins/default-ghost/sprite.png`
- `pages/index/composables/useSkins.js`

### 🆕 [近期更新] 分层模式设置开关
**状态**: 待验证 ✅
**测试要点**:
- 在 "设置" (Config) 面板找到 "分层宠物模式" 开关
- 切换开关验证是否实时生效
- 重启应用验证设置是否被持久化保存

**相关文件**: 
- `pages/index/components/ConfigPanel.vue`
- `pages/index/index.vue`
- `pages/index/composables/useIndexHandlers.js`

### 🆕 [近期更新] UI 交互修复 (认知核心/人格入口)
**状态**: 待验证 ✅
**测试要点**:
- 点击 ConfigPanel 中的 "认知核心" 正常跳转
- 点击 "宠物人格" 正常跳转
