...
**总计**: 17 个问题（13 已修复，1 修复中，2 未修复，1 待测试）

---

- [🔴 高优先级](#-高优先级影响核心功能) - #112, #109, #108, #107, #101, #102, #103, #104, #105
- [🟡 中优先级](#-中优先级影响用户体验) - #2
- [🟢 低优先级](#-低优先级轻微问题) - #1, #3, #8
- [🔍 代码审查](#-代码审查新发现-2025-12-06) - #NEW-1, #NEW-4, #NEW-6
...
**问题描述**:  
点击悬浮宠物后，悬浮窗会自动移动到屏幕右上角，无论用户之前将宠物拖拽到哪个位置。
...
**根本原因**:  
`showBubble()`/`hideBubble()` 中调用 `updateWindow()` 时，插件会重新应用初始的 `setGravity(2)` 设置（2 = 上右角）。

**修复方案**:  
1. 移除 `setGravity(2)` 调用 - 不再设置初始对齐位置
2. 移除所有 `updateWindow()` 调用 - 避免位置重置
3. 将 `setSidePattern` 改为 0（无吸附）
...
**问题描述**:  
召唤宠物后返回桌面，悬浮窗会在几秒后自动消失。

**期望行为**:  
悬浮窗应在桌面持续显示，直到用户主动收回。
...
**可能原因**:  
1. Android 系统后台杀死应用进程
2. `onHide` 生命周期触发了 `hideFloatWindow()`
3. 悬浮窗服务被系统回收

**相关文件**:
- `pages/index/index.vue` (L376-382: `onHide` 处理)
- `pages/index/composables/useFloatWindow.js`
...
### #110 【UI交互】配置面板"认知核心"和"宠物人格"点击无反应
**发现时间**: 2025-12-07
**严重程度**: 🔴 高
**状态**: ✅ 已修复
**测试设备**: Windows 模拟器
...
**问题描述**: 
点击 ConfigPanel 中的"认知核心"或"宠物人格"入口没有任何反应。

**根本原因**: 
`ConfigPanel.vue` 组件 methods 中缺失了 `navigateToCognitive` 和 `navigateToPersonality` 方法定义，尽管模板中通过 `@tap` 绑定了这些方法。可能是合并代码时遗漏。

**修复方案**: 
在 `ConfigPanel.vue` methods 中补充缺失的跳转方法。

**相关文件**:
- `pages/index/components/ConfigPanel.vue` (L227-248)
...
### 已完成
- ✅ #101, #102, #104, #105 - 悬浮窗相关
- ✅ #107, #108, #109 - 皮肤/日记系统
- ✅ #NEW-1, #NEW-4, #NEW-6 - 代码审查发现

---

**更新时间**: 2025-12-07  
**总数**: 15 个问题（12 已修复，1 修复中，1 未修复，1 待测试）
...
**验证确认**:
- `usePetInteraction.js:29-36` 接受 `growthInstance` 参数
- `index.vue:291-297` 正确传入共享实例
